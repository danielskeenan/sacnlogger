...
namespace sacnlogger
{
    namespace detail
    {
        ...
        etcpal::IpAddr ipAddrFromNetworkdJson(const nlohmann::json& family, const nlohmann::json& address)
        {
            if (family == 2)
            {
                // IPv4.
    ...
    void SystemConfig::readFromSystem()
    {
        ...
        if (addresses != eth0Info.end())
        {
            ...
            for (const auto [addressFamily, ipAddrType] : detail::kNetworkdAddressFamilies)
            {
                ...
                if (familyAddress != addresses.value().end())
                {
                    ...
                    newNetworkConfig.address =
                        detail::ipAddrFromNetworkdJson(addressFamily, familyAddress.value()["Address"]);
        ...
        {
            const auto addressFamily =
                std::ranges::find_if(
                    detail::kNetworkdAddressFamilies,
                    [&newNetworkConfig](const decltype(detail::kNetworkdAddressFamilies)::value_type& o)
                    { return o.second == newNetworkConfig.address.type(); })
                    ->first;
            ...
            {
                newNetworkConfig.gateway =
                    detail::ipAddrFromNetworkdJson(addressFamily, defaultRoute.value()["Gateway"]);
            }
        }
        ...
        if (ntp != eth0Info.end() && !ntp.value().empty())
        {
            ...
            {
                newNetworkConfig.ntpServer = detail::ipAddrFromNetworkdJson(ntpInfo["Family"], address.value());
            }
